...
- **Keypoint-visibility report** - the **Visibility** button charts the % of each event type's annotated frames in which each keypoint is visible, as a **heatmap or grouped bar chart**, optionally broken down **per track** (event × track), with a CSV download
