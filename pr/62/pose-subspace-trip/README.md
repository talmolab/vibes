...
A trippy variant of [Pose Subspace Analysis](../pose-subspace-analysis/). Instead of plotting your pose on a 2D PCA scatter, the PCA projection of your live pose **drives a real-time ray-marched fractal hyperspace** — you control the trip with your body.

Capture a few extreme poses to define your control axes (the principal components of your captured pose set). Then move through that subspace and watch the fractal warp: orbit, fold, twist, recolor, zoom, and re-symmetrize in response to where your body sits in pose-space.
...
4. **Drive** — the 6 normalized coordinates are smoothed and fed as uniforms into a WebGL fragment shader that ray-marches a [mandelbox](https://en.wikipedia.org/wiki/Mandelbox) fractal through a kaleidoscope.

### What each PC controls
...
Before you capture 3 poses, the visualization runs an **idle drift** and responds to a generic fallback signal (body centroid, size, and spread), so it's interactive immediately — but the real expressiveness comes from your custom PCA axes.
...
- **⛶ Fullscreen** — expand the hyperspace canvas
...
- **Rendering:** raw WebGL1 fullscreen-triangle fragment shader, ray-marched mandelbox distance estimator with kaleidoscopic domain folding and an [iq cosine palette](https://iquilezles.org/articles/palettes/)
